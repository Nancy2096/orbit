...
export default function EditClientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function fetchClient() {
    ...
    if (clientData) {
      ...
      // Determine country code - could be stored as code or name
      let countryCode = clientData.country || "MX"
      ...
      // Load location dropdowns based on saved data
      const states = getStatesByCountry(countryCode)
      setAvailableStates(states)
      
      if (clientData.state) {
        const cities = getCitiesByState(countryCode, clientData.state)
        setAvailableCities(cities)
        
        if (clientData.city) {
          const postalCodes = getPostalCodesByCity(countryCode, clientData.state, clientData.city)
          setAvailablePostalCodes(postalCodes)
        }
      }
      ...
      setFormData({
        ...
        exterior_number: clientData.exterior_number || "",
        interior_number: clientData.interior_number || "",
        neighborhood: clientData.neighborhood || "",
        between_streets: clientData.between_streets || "",
        address_references: clientData.address_references || "",
        ...
        tax_regime: clientData.tax_regime || "",
        cfdi_use: clientData.cfdi_use || "",
        primary_contact_name: clientData.primary_contact_name || "",
        primary_contact_position: clientData.primary_contact_position || "",
        ...
        payment_terms: clientData.payment_terms?.toString() || "30",
    ...
    if (contactsData) {
      setAdditionalContacts(contactsData.map(c => ({
        id: c.id,
        name: c.name,
        position: c.position || "",
        email: c.email || "",
  ...
  function saveContact() {
    ...
    if (editingContactIndex !== null) {
      ...
      updated[editingContactIndex] = { ...contactForm, id: updated[editingContactIndex].id }
      setAdditionalContacts(updated)
    } else {
      setAdditionalContacts([...additionalContacts, { ...contactForm, isNew: true }])
    }
  ...
  function deleteContact(index: number) {
    ...
    if (contact.id) {
      // Mark for deletion
      const updated = [...additionalContacts]
      updated[index] = { ...contact, isDeleted: true }
      setAdditionalContacts(updated)
    } else {
      // Remove from array
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    for (const contact of additionalContacts) {
      if (contact.isDeleted && contact.id) {
        // Soft delete
        await supabase.from("client_contacts").update({ is_active: false }).eq("id", contact.id)
              ...
              {visibleContacts.length === 0 ? (
                ...
                <div className="space-y-3">
                  {visibleContacts.map((contact, index) => (
                    <div key={contact.id || index} className="flex items-center justify-between p-3 rounded-lg border bg-muted/30">
                      ...
                      <div className="flex items-center gap-2">
                        <Button type="button" variant="ghost" size="sm" onClick={() => openEditContact(contact, additionalContacts.indexOf(contact))}>
                          Editar
                        </Button>
                        <Button type="button" variant="ghost" size="sm" onClick={() => deleteContact(additionalContacts.indexOf(contact))}>
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button type="button" onClick={saveContact} disabled={!contactForm.name.trim()}>
              {editingContactIndex !== null ? "Guardar Cambios" : "Agregar Contacto"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
