...
export default function OrganicSocialPage() {
  ...
  // Calculate totals
  const totalFollowers = mockSocialAccounts.reduce((acc, a) => acc + a.followers, 0)
  const avgEngagement = mockSocialAccounts.reduce((acc, a) => acc + a.engagementRate, 0) / mockSocialAccounts.length
  const totalReach = 485000
  const totalImpressions = 720000
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {mockSocialAccounts.slice(0, 4).map((account) => {
              ...
              return (
                <Link 
                  key={account.id} 
                  href={`/dashboard/marketing-intelligence/social/${account.network}`}
                  className="block"
                >
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" size="sm" asChild>
              <Link href="/dashboard/marketing-intelligence/calendar">
                Ver todas
              </Link>
            </Button>
          </div>
